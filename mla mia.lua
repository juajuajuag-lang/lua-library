...
local function ApplyKillerWarning(player,character)

    local key=PlayerKey(player)

    local name="KYS_KillerWarning_"..key
    ...
    local warning=ESPFolder:FindFirstChild(name)
    ...
    if not warning then
        ...
        warning.LightInfluence=0
        warning.MaxDistance=0
        warning.Size=UDim2.fromOffset(28,28)
        warning.StudsOffset=Vector3.new(0,4.2,0)
        warning.Parent=ESPFolder

        local image=Instance.new("ImageLabel")
        image.Name="Image"
        image.BackgroundTransparency=1
        image.Size=UDim2.fromScale(1,1)
        image.Parent=warning

    end

    warning.Adornee=head
    warning.Enabled=true

    local image=warning:FindFirstChild("Image")

    if image then

        -- Amarillo por defecto.
        -- Rojo cuando el killer está cerca.
        ...
        if myRoot and killerRoot then
            close=
                (myRoot.Position-killerRoot.Position).Magnitude<=30
        end

        image.Image=
            close
            and KILLER_WARNING_RED
            or KILLER_WARNING_YELLOW
    end
end
