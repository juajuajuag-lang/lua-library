...
do
    ...
    local function KYS_RegisterWorldDescendant(obj)
        ...
        local function classify(name)
            ...
            if n=="generator" or n:find("generator",1,true) or n:find("machine",1,true) then return "Generator" end
            if n=="hook" or n:find("hook",1,true) then return "Hook" end
            if n=="gate" or n:find("gate",1,true) or n:find("exit",1,true) then return "Gate" end
            if n=="window" or n:find("window",1,true) or n:find("vault",1,true) then return "Window" end
            if n=="pallet" or n=="palletwrong" or n:find("pallet",1,true) then return "Palletwrong" end
            if n:find("scp",1,true) or n:find("zombie",1,true) then return "SCPZombie" end
        end
        if obj:IsA("Model") then
            local cat=classify(obj.Name)
            if cat then KYS_EnsureWorldEntry(cat,obj) end
            return
        end
        if obj:IsA("BasePart") then
            ...
            while parent and parent~=Workspace do
                if parent:IsA("Model") then
                    local cat=classify(parent.Name)
                    if cat then KYS_EnsureWorldEntry(cat,parent);return end
                end
    ...
    local function KYS_RefreshESPRoots()
        ...
        local roots={}
        for _,name in ipairs({"Map","Map1"}) do
        ...
        if #roots==0 then
            for _,child in ipairs(Workspace:GetChildren()) do
                if child:IsA("Folder") or child:IsA("Model") then
                    ...
                    if n:find("map",1,true) or n:find("arena",1,true) then roots[#roots+1]=child end
                end
            end
        end
        for _,root in ipairs(roots) do KYS_AttachESPRoot(root) end
    end
    ...
    local function KYS_WorldClassify(obj)
    ...
    local function KYS_WorldScan()
        ...
        for _,root in ipairs(roots) do
            local cat=KYS_WorldClassify(root)
            if cat then KYS_EnsureWorldEntry(cat,root) end
            for _,obj in ipairs(root:GetDescendants()) do
                cat=KYS_WorldClassify(obj)
                if cat then KYS_EnsureWorldEntry(cat,obj) end
            end
        end
    end
                    ...
                    if enabled and KYS_ESPState.WorldMasterESP then
                        for model,entry in pairs(models) do
                            ...
                            elseif model and KYS_Alive(model) then
                                ...
                                if not KYS_ValidPart(part) then
                                    part=KYS_PickWorldPart(model,cat)
                                    if not KYS_ValidPart(part) then
                                        part=model:IsA("BasePart") and model or model:FindFirstChildWhichIsA("BasePart",true)
                                    end
                                    entry.part=part
                                end
                                if KYS_ValidPart(part) then
                                    local adorn=(model:IsA("Model") or model:IsA("BasePart")) and model or part
                                    local key=KYS_WorldKey(cat,model)
                                    KYS_EnsureHighlight(key.."_HL",adorn,color,false)
                        ...
                        for model in pairs(models) do KYS_ClearWorldVisual(cat,model) end
                    end
                end
                task.wait(0.35)
            end
...
local WarningEnabled=false
local WarningLoop=nil
local function IsKillerPlayer(p)
    return p and p.Team and p.Team.Name=="Killer"
end
local function IsSurvivorPlayer(p)
    return p and p.Team and p.Team.Name=="Survivors"
end
local function ClearKillerWarnings()
    for _,p in ipairs(Players:GetPlayers()) do
        local r=p.Character and p.Character:FindFirstChild("HumanoidRootPart")
        local w=r and r:FindFirstChild("KYS_SurvivorKillerWarn")
        if w then w:Destroy() end
    end
end
local function WarnImage(parent,name,image,visible,size,pos)
...
local function UpdateKillerWarnings()
    if not WarningEnabled then ClearKillerWarnings();return end
    local killers={}
    for _,p in ipairs(Players:GetPlayers()) do
        if IsKillerPlayer(p) and p.Character then
            local r=p.Character:FindFirstChild("HumanoidRootPart")
            local h=p.Character:FindFirstChildOfClass("Humanoid")
            if r and h and h.Health>0 then killers[#killers+1]=r end
        end
    end
    for _,p in ipairs(Players:GetPlayers()) do
        if IsSurvivorPlayer(p) and p.Character then
            local r=p.Character:FindFirstChild("HumanoidRootPart")
            local h=p.Character:FindFirstChildOfClass("Humanoid")
            if r and h and h.Health>0 then
                local d=math.huge
                for _,kr in ipairs(killers) do d=math.min(d,(r.Position-kr.Position).Magnitude) end
                local w=r:FindFirstChild("KYS_SurvivorKillerWarn")
                if d<=60 then
                    local danger=d<=40
                    if not w then
                        w=Instance.new("BillboardGui")
                        w.Name="KYS_SurvivorKillerWarn"
                        w.Adornee=r
                        w.AlwaysOnTop=true
                        w.MaxDistance=2000
                        w.Parent=r
                        local f=Instance.new("TextLabel")
                        f.Name="FallbackLabel"
                    ...
                    w.Size=danger and UDim2.fromOffset(76,56) or UDim2.fromOffset(56,56)
                    WarnImage(w,"YellowIcon",Images.KillerWarningYellow,true,danger and UDim2.fromScale(.62,1) or UDim2.fromScale(1,1),UDim2.fromScale(0,0))
                    WarnImage(w,"PurpleIcon",Images.KillerWarningPurple,danger,UDim2.fromScale(.62,1),UDim2.fromScale(.38,0))
                    local f=w:FindFirstChild("FallbackLabel")
...
local function StartKillerWarning()
    WarningEnabled=true
    if WarningLoop then return end
    WarningLoop=game:GetService("RunService").Heartbeat:Connect(UpdateKillerWarnings)
end
local function StopKillerWarning()
    WarningEnabled=false
    if WarningLoop then WarningLoop:Disconnect();WarningLoop=nil end
    ClearKillerWarnings()
end
