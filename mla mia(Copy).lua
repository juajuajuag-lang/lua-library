...
-- =====================================================
-- SURVIVOR KILLER WARNING
-- SOLO funciona cuando el jugador local es Survivor
-- =====================================================

local WARNING_YELLOW="rbxassetid://120753562298647"
local WARNING_RED="rbxassetid://99499680507460"
...
local function ClearKillerWarning()
    if WarningGui then
        pcall(function()
            WarningGui:Destroy()
        end)
    ...
    if root then
        local old=root:FindFirstChild("KYS_SurvivorKillerWarn")
        if old then
            old:Destroy()
        end
    end
...
local function GetNearestKillerDistance()
    ...
    for _,player in ipairs(Players:GetPlayers()) do
        if player~=LP and GetRole(player)=="Killer" then
            ...
            if killerRoot and hum and hum.Health>0 then
...
local function CreateKillerWarning()
    ...
    local existing=root:FindFirstChild("KYS_SurvivorKillerWarn")

    if existing then
        return existing
    end

    local warn=Instance.new("BillboardGui")
    warn.Name="KYS_SurvivorKillerWarn"
    warn.Adornee=root
    warn.AlwaysOnTop=true
    warn.Size=UDim2.fromOffset(76,44)
...
local function UpdateKillerWarning()
    ...
    local distance=GetNearestKillerDistance()

    -- Killer demasiado lejos o inexistente
    if distance>WARNING_MAX_DISTANCE then
        ClearKillerWarning()
        return
    end

    local warn=CreateKillerWarning()

    if not warn then
        return
    end

    local image=warn:FindFirstChild("WarningImage")

    if image then
        if distance<=WARNING_DANGER_DISTANCE then
            image.Image=WARNING_RED
        else
            image.Image=WARNING_YELLOW
        end
    end
...
local function StartKillerWarning()
    ...
    task.spawn(function()
        ...
        while WarningRunning do

            pcall(UpdateKillerWarning)
...
-- Arrancar el sistema independientemente del Player ESP
StartKillerWarning()
...
local function WorldValid(obj)
    return obj and obj.Parent~=nil
end

local function WorldPart(model)
    if not WorldValid(model) then
        return nil
    end
    ...
    return model.PrimaryPart
        ...
        or model:FindFirstChild("HookPoint",true)
...
local function WorldKey(cat,model)
    return "KYS_World_"
        ..cat
        .."_"
        ..tostring(model):gsub("[^%w]","_")
end

local function WorldClear(cat,model)
...
local function WorldRemove(cat,model)
    if WorldReg[cat] then
        WorldClear(cat,model)
        WorldReg[cat][model]=nil
    end
end

local function WorldAdd(cat,model)
    if not WorldValid(model) then
        return
    end

    if not WorldReg[cat] then
        return
    end
...
local function AttachWorldRoot(root)
    ...
    WorldConnections[root]={
        root.DescendantAdded:Connect(RegisterWorld),
        root.DescendantRemoving:Connect(UnregisterWorld)
    }
...
local function RefreshWorldRoots()
    ...
    for root,connections in pairs(WorldConnections) do
...
local function WorldEnabled(cat)
    ...
    if selected[name[cat]]~=nil then
        return selected[name[cat]]==true
    end
...
local function ApplyWorld(cat,model,entry)
...
print("[KysHub] World ESP logic loaded")