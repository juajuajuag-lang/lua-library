...
for _,obj in ipairs(Lighting:GetChildren()) do
    ...
    or obj:IsA("SunRaysEffect") then
        ...
        if obj:IsA("Atmosphere") then
            ...
            OriginalEffects[obj.ClassName].Density=obj.Density
            OriginalEffects[obj.ClassName].Offset=obj.Offset
            OriginalEffects[obj.ClassName].Glare=obj.Glare
            OriginalEffects[obj.ClassName].Haze=obj.Haze

        elseif obj:IsA("BloomEffect") then

            OriginalEffects[obj.ClassName].Intensity=obj.Intensity
            OriginalEffects[obj.ClassName].Size=obj.Size
            OriginalEffects[obj.ClassName].Threshold=obj.Threshold
        ...
        elseif obj:IsA("ColorCorrectionEffect") then
            ...
            OriginalEffects[obj.ClassName].Brightness=obj.Brightness
            OriginalEffects[obj.ClassName].Contrast=obj.Contrast
            OriginalEffects[obj.ClassName].Saturation=obj.Saturation
            OriginalEffects[obj.ClassName].TintColor=obj.TintColor

        elseif obj:IsA("SunRaysEffect") then

            OriginalEffects[obj.ClassName].Intensity=obj.Intensity
            OriginalEffects[obj.ClassName].Spread=obj.Spread
        end
    end
end
...
local WeatherValues={
    Default={
        Brightness=LightingOriginal.Brightness,
        ClockTime=LightingOriginal.ClockTime,
        FogStart=LightingOriginal.FogStart,
        FogEnd=LightingOriginal.FogEnd,
        OutdoorAmbient=LightingOriginal.OutdoorAmbient,
        Ambient=LightingOriginal.Ambient
    },
    ...
    Night={
        ...
        FogStart=0,
        FogEnd=100000,
        OutdoorAmbient=Color3.fromRGB(45,45,65),
...
local function ApplyWeather()
    ...
    local theme=State.Lighting.Weather or "Default"

    if State.Lighting.DisableWeather then
        theme="Default"
    end

    local preset=WeatherValues[theme]

    if not preset then
        preset=WeatherValues.Default
    end
...
local function RemoveCurrentCustomSky()

    local sky=Lighting:FindFirstChild("KysHub_CustomSky")
...
local function ApplySky()
    ...
    local selected=State.Lighting.Sky or "Default"

    RemoveCurrentCustomSky()
    ...
    sky.Name="KysHub_CustomSky"

    if selected=="Blue" then
    ...
    elseif selected=="Purple" then

        sky.SkyboxBk="rbxassetid://271042516"
        sky.SkyboxDn="rbxassetid://271042556"
        sky.SkyboxFt="rbxassetid://271042556"
        sky.SkyboxLf="rbxassetid://271042556"
        sky.SkyboxRt="rbxassetid://271042556"
        sky.SkyboxUp="rbxassetid://271042556"
...
local function ApplyFullBright()
    ...
    if not State.Lighting.FullBright then
        return
    end
...
local function ApplyNoFog()
    ...
    pcall(function()
        ...
        if State.Lighting.NoFog then
            Lighting.FogStart=0
            Lighting.FogEnd=100000
        else
            Lighting.FogStart=LightingOriginal.FogStart
...
local function ApplyNoShadows()
    ...
    pcall(function()
        ...
        if State.Lighting.NoShadows then
            Lighting.GlobalShadows=false
        else
...
local function ApplyPostEffects()
    ...
    -- COLOR CORRECTION
    do
        local data=OriginalEffects.ColorCorrectionEffect
        local obj=data and data.Object

        if obj and obj.Parent then

            if State.Lighting.NoColorCorrection then
                obj.Enabled=false
            else
    ...
    -- ATMOSPHERE
    do
        local data=OriginalEffects.Atmosphere
        local obj=data and data.Object

        if obj and obj.Parent then

            if State.Lighting.RemoveAtmosphere then
    ...
    -- BLOOM
    do
        local data=OriginalEffects.BloomEffect
        local obj=data and data.Object

        if obj and obj.Parent then

            if State.Lighting.RemoveBloom then
                obj.Enabled=false
            else
                obj.Enabled=data.Enabled
            end
    ...
    -- BLUR
    do
        local data=OriginalEffects.BlurEffect
        local obj=data and data.Object

        if obj and obj.Parent then

            if State.Lighting.RemoveBlur then
                obj.Enabled=false
                obj.Size=0
            else
                obj.Enabled=data.Enabled
...
local function RestoreLighting()
    ...
    pcall(function()
        ...
        Lighting.ClockTime=
            LightingOriginal.ClockTime

        Lighting.FogStart=
            LightingOriginal.FogStart
        ...
        Lighting.OutdoorAmbient=
            LightingOriginal.OutdoorAmbient

        Lighting.Ambient=
            LightingOriginal.Ambient
    end)

    for _,data in pairs(OriginalEffects) do
...
local LightingLoop=task.spawn(function()
...
print("[KysHub] Lighting Logic loaded")

--==================================================
-- CAMERA + LIGHTING LOGIC
-- ADAPTADO A LA UI LOCAL EXISTENTE
-- NO MODIFICA LA UI
--==================================================
...
local OriginalZoomMin=nil
local OriginalZoomMax=nil
local ZoomSaved=false
...
local function UpdateCameraFOV()
    ...
    if State.Camera.FOVEnabled then
        ...
        if not CameraFOVActive then

            CameraOriginalFOV=cam.FieldOfView
            CameraFOVActive=true

        end

        cam.FieldOfView=
            State.Camera.FOV or 70

    elseif CameraFOVActive then

        if CameraOriginalFOV then
            cam.FieldOfView=CameraOriginalFOV
        end
...
local function UpdateInfinityZoom()
    ...
    if State.Camera.InfinityZoom then
        ...
        if OriginalZoomMin then
            LP.CameraMinZoomDistance=
                OriginalZoomMin
        end

        if OriginalZoomMax then
            LP.CameraMaxZoomDistance=
                OriginalZoomMax
        end
    end
...
local function UpdateNoCutscene()
...
for _,obj in ipairs(Lighting:GetChildren()) do
    ...
    or obj:IsA("SunRaysEffect") then
        ...
        if obj:IsA("Atmosphere") then
            ...
            OriginalEffects[obj].Density=obj.Density
            OriginalEffects[obj].Offset=obj.Offset
            OriginalEffects[obj].Glare=obj.Glare
            OriginalEffects[obj].Haze=obj.Haze

        elseif obj:IsA("BloomEffect") then

            OriginalEffects[obj].Intensity=obj.Intensity
            OriginalEffects[obj].Size=obj.Size
            OriginalEffects[obj].Threshold=obj.Threshold
        ...
        elseif obj:IsA("ColorCorrectionEffect") then
            ...
            OriginalEffects[obj].Brightness=obj.Brightness
            OriginalEffects[obj].Contrast=obj.Contrast
            OriginalEffects[obj].Saturation=obj.Saturation
            OriginalEffects[obj].TintColor=obj.TintColor

        elseif obj:IsA("SunRaysEffect") then

            OriginalEffects[obj].Intensity=obj.Intensity
            OriginalEffects[obj].Spread=obj.Spread
...
--==================================================
-- LIGHTING EFFECT HELPERS
--==================================================

local function SetEffectEnabled(className,enabled)
...
local function UpdateFullBright()
    ...
    if State.Lighting.FullBright then
...
local function UpdateNoFog()
    ...
    if State.Lighting.NoFog then
        ...
        for obj,data in pairs(OriginalEffects) do

            if obj
            and obj.Parent
            and obj:IsA("Atmosphere") then
...
local function UpdateNoColorCorrection()

    SetEffectEnabled(
        "ColorCorrectionEffect",
        not State.Lighting.NoColorCorrection
    )
end
    ...
    for _,obj in ipairs(Lighting:GetChildren()) do
        ...
        if obj:IsA("Atmosphere") then
            ...
            if State.Lighting.RemoveAtmosphere then
                ...
                local data=OriginalEffects[obj]
...
local function UpdateWeather()
    ...
    local preset=
        WeatherPresets[theme]
        or WeatherPresets.Default
...
local CameraLightingConnection

if getgenv().KYS_CameraLightingConnection then

    pcall(function()
        getgenv().KYS_CameraLightingConnection:Disconnect()
    end)

end

CameraLightingConnection=
    ...
    function()
        ...
        pcall(UpdateShiftLock)
        pcall(UpdateInfinityZoom)
        pcall(UpdateNoCutscene)
...
getgenv().KYS_CameraLightingConnection=
    CameraLightingConnection

--==================================================
-- CAMERA RESET AFTER RESPAWN
--==================================================
...
print("[KysHub] Camera + Lighting logic loaded")