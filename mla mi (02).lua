...
--==================================================
-- LIGHTING ENGINE
-- INTEGRADO CON VisualState.Lighting
--==================================================
...
for _,obj in ipairs(Lighting:GetChildren()) do
    ...
    or obj:IsA("Sky") then
        ...
        LightingOriginalObjects[obj]={
            Parent=obj.Parent,
            Enabled=obj:IsA("PostEffect")
                and obj.Enabled
                or nil
        }
...
local LightingEngine={
    AppliedWeather="Default",
    AppliedSky="Default"
}

local function RestoreLightingObject(obj,data)
...
local function UpdateFullBright()
    ...
    if VisualState.Lighting.FullBright then
...
local function UpdateNoFog()
    ...
    if VisualState.Lighting.NoFog then
        ...
        Lighting.FogColor=LightingOriginal.FogColor
...
local function UpdateNoShadows()
    ...
    if VisualState.Lighting.NoShadows then
...
local function UpdateNoColorCorrection()
    ...
    if VisualState.Lighting.NoColorCorrection then
        ...
        for obj,data in pairs(LightingOriginalObjects) do

            if obj:IsA("ColorCorrectionEffect") then
                RestoreLightingObject(obj,data)
            end
...
local function UpdateNoAtmosphere()

    if VisualState.Lighting.RemoveAtmosphere then
...
local function UpdateNoBloom()

    if VisualState.Lighting.RemoveBloom then
...
local function UpdateNoBlur()
    ...
    if VisualState.Lighting.RemoveBlur then
...
local WeatherPresets={
    ...
    Default={
        ...
        FogColor=LightingOriginal.FogColor,
    ...
    Storm={
        ...
        FogColor=Color3.fromRGB(70,75,85),
        FogEnd=400
    }
}
...
local function ApplyWeather()
    ...
    local name=VisualState.Lighting.Weather or "Default"

    if VisualState.Lighting.DisableWeather then
        name="Default"
    end
    ...
    if not preset then
        preset=WeatherPresets.Default
        name="Default"
    end

    LightingEngine.AppliedWeather=name

    if not VisualState.Lighting.FullBright then
    ...
    if not VisualState.Lighting.NoFog then
...
local SkyPresets={
    ...
    Blue={
        ClockTime=13,
        Ambient=Color3.fromRGB(90,110,150),
        OutdoorAmbient=Color3.fromRGB(110,140,190)
    },

    Purple={
        ClockTime=20,
        Ambient=Color3.fromRGB(90,45,120),
        OutdoorAmbient=Color3.fromRGB(110,60,150)
    },
...
local function ApplySky()
    ...
    local name=VisualState.Lighting.Sky or "Default"
    ...
    LightingEngine.AppliedSky=name
...
local function ApplyLighting()
    ...
    pcall(UpdateNoFog)
    pcall(UpdateNoShadows)
    pcall(UpdateNoColorCorrection)
    pcall(UpdateNoAtmosphere)
    pcall(UpdateNoBloom)
    pcall(UpdateNoBlur)
    pcall(ApplyWeather)
    pcall(ApplySky)
...
Lighting.ChildAdded:Connect(function(obj)
    ...
    task.defer(function()
        ...
        pcall(function()
            ...
            and VisualState.Lighting.NoColorCorrection then

                obj.Enabled=false

            elseif obj:IsA("BloomEffect")
            and VisualState.Lighting.RemoveBloom then

                obj.Enabled=false

            elseif obj:IsA("BlurEffect")
            and VisualState.Lighting.RemoveBlur then

                obj.Enabled=false

            elseif obj:IsA("Atmosphere")
            and VisualState.Lighting.RemoveAtmosphere then
...
print("[KysHub] Lighting Engine loaded")
